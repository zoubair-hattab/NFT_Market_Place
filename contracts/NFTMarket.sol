...
contract NFTMarket is ReentrancyGuard {
    ...
    // we can use Ownable contract on openzipline
    modifier onlyOwner() {
        require(
            msg.sender == owner,
    ...
    /**
     * @notice This function is for creating and selling an NFT.
     * @param uri Refers to the location where metadata has been stored.
     * @param _price Refers to the price for the NFT.
     * @param _nftContract Refers to adding the NFT token.
     */
    function sellItem(
        string memory uri,
        uint256 _price,
        ...
        uint256 _tokenId = NFTToken(_nftContract).safeMint(
            uri,
            address(this),
            msg.sender
        );
    ...
    /**
     * @notice This function is for canceling the sale of an NFT.
     * @param _tokenId Refers to the ID of the NFT.
     */
    ...
    /**
     * @notice This function is for buy   an NFT and send the price to creator  this NFT.
     * @param _tokenId Refers to the ID of the NFT.
     */
    ...
    function withdraw(address payable recipient) external onlyOwner {
        require(
            recipient != address(0),
            "The recipient's address should not be equal to address zero."
        );
        recipient.transfer(address(this).balance);
    }

    function changeCostListing(uint256 _minCostLising) external onlyOwner {
        mintingCost = _minCostLising;
    }
}
